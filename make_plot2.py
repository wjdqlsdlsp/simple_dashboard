...
for user, color in zip(users,colors):
    ...
    date = data_user.tail(1)['date'].values[0] + pd.to_timedelta('1.5 days')
    plt.text(s=f'{user}', x=date, y = time,font=font, fontsize=35, va='center', ha='center', color=color)
    plt.text(s=f'{round(time,2)}시간', x=date, y=time-10,font=font, fontsize=25, va='center', ha='center', color=color)
...
plt.title(f'Total 공부시간 ({end} ~ {first})', font=font, fontsize= 40,pad=32)
